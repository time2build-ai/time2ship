...
export class TokenService {
  ...
  async verifyRefreshToken(token: string): Promise<string> {
    ...
    try {
      ...
      throw new UnauthorizedError('Invalid refresh token', AUTH_ERROR_CODES.REFRESH_TOKEN_INVALID);
    }
    ...
    if (!storedToken) {
      throw new UnauthorizedError('Refresh token not found', AUTH_ERROR_CODES.REFRESH_TOKEN_INVALID);
    }

    if (storedToken.isRevoked) {
      throw new UnauthorizedError('Refresh token has been revoked', AUTH_ERROR_CODES.REFRESH_TOKEN_INVALID);
    }

    if (new Date() > storedToken.expiresAt) {
      throw new UnauthorizedError('Refresh token expired', AUTH_ERROR_CODES.REFRESH_TOKEN_INVALID);
    }
  ...
  verifyAccessToken(token: string): { userId: string; email: string } {
    try {
      ...
      throw new UnauthorizedError('Invalid token', AUTH_ERROR_CODES.TOKEN_INVALID);
    }
  }
}
